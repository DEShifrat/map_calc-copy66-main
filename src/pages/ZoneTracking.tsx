...
import Polygon from 'ol/geom/Polygon'; // Импортируем Polygon для работы с зонами
  ...
  const handleAutoCalculateZones = () => {
    ...
    for (let y = 0; y < mapHeightMeters; y += zoneSizeInput) {
      for (let x = 0; x < mapWidthMeters; x += zoneSizeInput) {
        ...
        // Create an OpenLayers Polygon object to get the interior point
        ...
        newZones.push({
          ...
          beaconCount: 1, // Set to 1 as per request
        });
    ...
    actions.setBeacons([...beacons, ...autoGeneratedBeacons]); // Add new beacons to existing ones
